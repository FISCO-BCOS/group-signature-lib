...
std::string ToBase64(std::string const &_in)
{
    bytes bs;
    for (size_t i = 0; i < _in.length(); i++)
    {
        bs.push_back((byte)_in[i]);
    }
    bytesConstRef _bin(&bs);
...
std::string FromBase64(std::string const &encoded_string)
{
    ...
    while (in_len-- && encoded_string[in_] != '=' && is_base64(encoded_string[in_]))
    {
        ...
        if (i == 4)
        {
            ...
            for (i = 0; (i < 3); i++)
                oriBytes.push_back(char_array_3[i]);
            i = 0;
        }
    }
    ...
    if (i)
    {
        ...
        for (j = 0; j < i - 1; j++)
            oriBytes.push_back(char_array_3[j]);
    }
